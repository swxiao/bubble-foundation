...
public class PKUtil {
	...
	/**
	 * 获得主键生成器实例,校验位通过参数设置 单机环境下,应该保证用相同的 suffix 在集群环境中,不同的机器必须提供不同的 suffix
	 * 来保证生成的ID的唯一性
	 * 
	 * @param suffix
	 *            唯一标识
	...
	/**
	 * Long.MAXVALUE-timestamp(19位)+url(42定长)+longPk(19位)
	 * 
	 * @param dateString
	 * @param url
	 * @return
	 */
	public String generateRowKey(String url, String dateString) {
		Assert.notNull(dateString, "The param of 'dateString' can't be null!");
		Assert.notNull(dateString, "The param of 'url' can't be null!");
		int pos = url.indexOf("?");
		if (pos > 0) {
			url = url.substring(0, pos);
		}
		if (url.length() > 1 && url.trim().endsWith("/")) {
			url = url.substring(0, url.length() - 1);
		}
		if (url.contains("//")) {
			url = url.replaceAll("//", "/");
		}
		String prefix = StringUtil.getRowKeyPrefix(url);
		StringBuilder stringBuilder = new StringBuilder();
		stringBuilder.append(prefix);
		stringBuilder.append(DateUtil.format("yyyyMMddHHmmdd", DateUtil.parse("yyyy-MM-dd HH:mm:dd", dateString)));
		String pk = String.valueOf(longPK());
		stringBuilder.append(pk.substring(12, pk.length()));
	 ...
	 * @return
	 */
	public String generateRowKey(String prefix, Date date) {
		StringBuilder stringBuilder = new StringBuilder();
		stringBuilder.append(StringUtil.getRowKeyPrefix(prefix));
		stringBuilder.append(DateUtil.format("yyyyMMddHHmmdd", date));
		String pk = String.valueOf(longPK());
		stringBuilder.append(pk.substring(12, pk.length()));
		return stringBuilder.toString();
	}
